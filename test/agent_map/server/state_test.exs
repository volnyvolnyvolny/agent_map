defmodule AgentMapServerStateTest do
  ...
  test "fetch" do
    ...
    refute(worker?(state, :a))
    refute(worker?(state, :b))
    assert(worker?(state, :c))
    assert(worker?(state, :d))
    assert(worker?(state, :e))
  end
  ...
  test "broadcast" do
    ...
    broadcast(state, [:a, :b, :c, :d, :e], %{action: :get_and_update, fun: &{:ok, &1 + 1}})
    :timer.sleep(1)
    assert(take(state, [:a, :b, :c, :d, :e]) == %{a: 42, b: 2, c: 5})
  end
end
